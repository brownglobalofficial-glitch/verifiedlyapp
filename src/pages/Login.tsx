...
const Login = () => {
  ...
  const continueAfterLogin = async (userId: string) => {
    ...
    if (!profile?.onboarding_completed) {
      navigate(`/onboarding${nextPath ? `?returnTo=${encodeURIComponent(nextPath)}` : ""}`, { replace: true });
      return;
    }
    ...
    try {
      ...
      if (!email.includes("@") || email.startsWith("@")) {
        ...
        if (lookupError || !lookup?.email) {
          throw new Error("No account was found with that handle. Try your email address.");
        }
        email = lookup.email;
      }
      ...
      if (!data.user) throw new Error("The account could not be loaded.");
      await continueAfterLogin(data.user.id);
    } catch (error: unknown) {
      toast({
        title: "Login failed",
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    ...
    window.sessionStorage.setItem(AUTH_NEXT_STORAGE_KEY, nextPath || "/dashboard");
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-10 bg-background">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          ...
          {!isPending && (
            <>
              <h1 className="text-2xl font-display font-bold">Sign in with Verifiedly</h1>
              <p className="text-sm text-muted-foreground mt-1">One account for your official profile and connected BrownGlobal apps.</p>
            </>
          )}
        </div>
          ...
          <>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                ...
                <Input id="identifier" type="text" autoComplete="username" placeholder="you@example.com or @yourhandle" value={identifier} onChange={(event) => setIdentifier(event.target.value)} required />
              </div>
              <div>
                <Label htmlFor="password">Password</Label>
                <div className="relative">
                  <Input id="password" type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} required className="pr-10" />
        ...
        <p className="text-center text-sm text-muted-foreground">
          New to Verifiedly? <Link to={`/signup${nextPath ? `?next=${encodeURIComponent(nextPath)}` : ""}`} className="text-foreground underline">Create an account</Link>
        </p>
      </div>
    </div>
  );
