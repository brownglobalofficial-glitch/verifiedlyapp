...
type VerificationProfile = { id: string; username: string; id_verified: boolean; verified_at: string | null };
...
const Verification = () => {
  ...
  const load = useCallback(async (checkProvider = false) => {
    ...
    const [profileResult, billingResult] = await Promise.all([
      ...
      supabase.from("verifiedly_billing").select("identity_status, identity_attempt_count, verification_payment_status").eq("user_id", session.user.id).maybeSingle(),
    ]);
  ...
  const startCheckout = async () => {
    setAction("checkout");
    try {
      const { data, error } = await supabase.functions.invoke("create-identity-checkout", { body: {} });
      if (error || data?.error) throw new Error(await readFunctionError(error, data));
      if (data?.already_verified || data?.already_paid) return void await load(false);
      if (data?.url) window.location.assign(data.url);
    } catch (error) {
      toast({ title: "Checkout could not start", description: error instanceof Error ? error.message : "Please try again.", variant: "destructive" });
    } finally {
      setAction(null);
    }
  };
  ...
  useEffect(() => {
    const identityReturned = searchParams.get("identity") === "returned";
    void (async () => {
      await load(identityReturned);
      if (identityReturned) setSearchParams({}, { replace: true });
    })();
  ...
  const paid = state.verification_payment_status === "paid";
      ...
      <div className="mx-auto max-w-2xl px-4 py-6 sm:py-8">
        {verified ? (
          <Card className="rounded-3xl border-foreground/10 p-8 text-center shadow-sm sm:p-10">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Stripe Identity successfully verified the account holder's identity. The badge confirms identity only; it does not verify Work, Education or every profile claim.</p>
          ...
          <Card className="overflow-hidden rounded-3xl border-foreground/10 shadow-sm">
            <div className="border-b bg-foreground px-6 py-7 text-background sm:px-8">
              <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-background/10"><ShieldCheck className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.18em] opacity-65">One-time verification</p><h1 className="mt-1 text-2xl font-display font-bold">Get Identity Verified</h1></div></div>
            </div>
            <div className="space-y-6 p-6 sm:p-8">
              <div><p className="text-3xl font-display font-bold">$9.99<span className="ml-1 text-sm font-normal text-muted-foreground">one time</span></p><p className="mt-1 text-xs text-muted-foreground">Your Verifiedly profile is free. This one-time payment starts the identity-verification process.</p></div>
              <ul className="grid gap-3 text-sm sm:grid-cols-2">{["Stripe Identity secure verification", "Identity Verified badge after approval", "Verification remains on your profile", "No Verifiedly subscription required"].map((item) => <li className="flex items-center gap-2" key={item}><Check className="h-4 w-4" />{item}</li>)}</ul>
              {checkoutReturned && <div className="flex gap-3 rounded-2xl border p-4"><Clock3 className="mt-0.5 h-4 w-4 shrink-0" /><p className="text-sm">Your payment is being confirmed. If the button below still shows checkout, wait a moment and refresh the status.</p></div>}
              <Button className="h-12 w-full rounded-xl" onClick={() => void startCheckout()} disabled={action !== null}>{action === "checkout" ? "Opening secure checkout…" : <>Pay $9.99 & continue <ExternalLink className="ml-2 h-4 w-4" /></>}</Button>
              {checkoutReturned && <Button className="w-full gap-2" variant="outline" onClick={refresh} disabled={action === "refresh"}><RefreshCw className={`h-4 w-4 ${action === "refresh" ? "animate-spin" : ""}`} />Refresh payment status</Button>}
              <p className="text-center text-xs leading-relaxed text-muted-foreground">The payment is for the identity-verification process. The Identity Verified badge appears only after successful verification.</p>
            </div>
          </Card>
        ) : (
          <Card className="overflow-hidden rounded-3xl border-foreground/10 shadow-sm">
            <div className="border-b bg-foreground px-6 py-7 text-background sm:px-8">
              <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-background/10"><ShieldCheck className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.18em] opacity-65">Payment confirmed</p><h1 className="mt-1 text-2xl font-display font-bold">Verify your identity</h1><p className="mt-1 text-xs opacity-70">Stripe Identity performs the ID and selfie check.</p></div></div>
            </div>
            <div className="space-y-6 p-6 sm:p-8">
              <div><p className="text-2xl font-display font-bold">Ready for secure verification</p><p className="mt-1 text-xs text-muted-foreground">Your one-time verification payment has been recorded.</p></div>
              ...
              <p className="text-center text-xs leading-relaxed text-muted-foreground">Stripe handles the identity check. Verifiedly receives the verification result and does not ordinarily store copies of your ID or selfie.</p>
            </div>
          </Card>
        )}
      </div>
